...
//echo "<br/><br/><br/>".$insert_ID."<br/>";
$_SESSION["sales_invoice_id"] = 'Inv-'.$insert_ID;
unset($_SESSION['id']);
...
$sql_customers = "INSERT INTO customers_data ( USERNAME, INVOICE_ID ,  _NAME, MOBILE, _ADDRESS, _STATE, MODE, total_qty, total_amount, Round_Off, grand_total ) VALUES ('".$_SESSION['USERNAME']."', '".$_SESSION['sales_invoice_id']."' , '$cus_name' , '$cus_mobile' , '$cus_address' , '$cus_state' , '$PAYMENT_MODE' , '$total_qty' , '$total_amount' , '$Round_Off' , '$grand_total')";
...
$sql_order_details  = "INSERT INTO order_data (cus_id , USERNAME, ITEM, HSN, QTY, RATE, TOTAL) VALUES ('$last_id' , '".$_SESSION['USERNAME']."' , '$item' , '$hsn' , '$qty' , '$rate' , '$total' )";
...
html += "<select class='form-control text-capitalize' id='colTwo" + count + "' name='colTwo[]' data-type='productName' required onchange='selectoption(this)'>";
  <?php
  $sql = "SELECT product,HSN FROM item_table_user WHERE USERNAME = '".$_SESSION['USERNAME']."' ORDER BY(product) ASC";
  ...
  echo 'html += "<option value='.$itm_row["HSN"].' >'.$itm_row["product"].'</option>" ;';
  }
...
html += "<td><input type='text' class='form-control' data-type='productTotal' id='colSix"+count+"' name='colSix[]' value='' tabindex='-1' readonly /></td>";
html += "</tr>";
document.getElementById("mytable").insertRow().innerHTML = html;
}
</script>
<script src="sales_invoice.js"></script>
<?php
mysqli_close($conn);
?>